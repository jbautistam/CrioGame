...
namespace ArkanoidGame.Model.Entities
{
	...
	{ // Enumerados privados
		...
		public PaddleModel(IView objView, GameObjectDimensions objDimensions) : base(objView, TimeSpan.Zero, objDimensions)
		{ StartPosition = new Vector2D(objDimensions.Position.X, objDimensions.Position.Y);
			CollisionEvaluator = new CollisionTargets(this, 
																								...
																								(int) (GroupCollisionObjects.Ball | GroupCollisionObjects.Enemy | GroupCollisionObjects.Pill));
		}
		...
		{	View.AddEntity(Scenes.GameScene.LayerGame, 
										 new LaserModel(View, 
																		(int) GroupCollisionObjects.Player,
																		(int) (GroupCollisionObjects.Enemy | GroupCollisionObjects.Brick),
																		new GameObjectDimensions(objContext.MathHelper.Clamp(fltX, 0, View.ViewPortScreen.Width),
																														 objContext.MathHelper.Clamp(Dimensions.Position.Y + Dimensions.ScaledDimensions.Height / 2, 
																																												 0, View.ViewPortScreen.Height)),
																		new Vector2D(0, -10), "Laser", Program.TimeSpanPlayerLaserUpdate));
		}
