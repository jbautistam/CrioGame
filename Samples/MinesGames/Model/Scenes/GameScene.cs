...
using MinesGame.Model.Entities;

namespace MinesGame.Model.Scenes
{
	/// <summary>
		...
		{	ScoresModel objScore = new ScoresModel(0, 3);
				...
				// Añade el fondo parallax
					ViewDefault.AddEntity(Layer.Background.ToString(), 
																new BackgroundParallaxEntity(ViewDefault, "Parallax1", Program.TimeSpanParallax,
																														 -2, 0, 0, 1));
					ViewDefault.AddEntity(Layer.Background.ToString(), 
																new BackgroundParallaxEntity(ViewDefault, "Parallax2", Program.TimeSpanParallax,
																														 -4, 0, 0, 2));
				// Añade los textos
					ViewDefault.AddEntity(Layer.UserInterface.ToString(), 
																new UserInterfaceModel(ViewDefault, objScore, Program.TimeSpanUserInterface));
				// Añade las entidades
					ViewDefault.AddEntity(LayerGame, new PlayerModel(ViewDefault, new GameObjectDimensions(0, 0)));
					ViewDefault.AddEntity(LayerGame, new EnemySpawner(ViewDefault, Program.TimeSpawnMine));
		}
